...
public class LightCone extends SlimefunItem implements NotPlaceable, Rechargeable {
    public static final float CAPACITY = 500.0F;
    ...
    public void CauseDamageToEntity(PlayerInteractEvent event, Integer damage) {
        event.getPlayer().sendMessage(ChatColors.color("&7测试:&fGun:&fLightCone:&fDamage" + damage));
        getLogger().info("&7测试:&fGun:&fLightCone:&fDamage" + damage);
    }
}
